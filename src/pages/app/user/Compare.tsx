...
export default function Compare() {
  ...
  useEffect(() => {
    const fetchSessions = async () => {
      ...
      setLoading(true);
      try {
        const resp = await securedApi.get<Session[]>(
  ...
  useEffect(() => {
    const fetchComparison = async () => {
      ...
      setLoading(true);
      try {
        const res = await securedApi.get<CompareResult>(
                  ...
                  <Card key={side.session_id} className="shadow">
                    <CardContent>
                      ...
                      <p className="text-sm text-gray-600 mb-2">
                        Taken on {new Date(side.created_at).toLocaleString()}
                      </p>
                      <p className="mb-4">
                        <strong>Overall Score:</strong> {side.overall_score} / 5
                      </p>
